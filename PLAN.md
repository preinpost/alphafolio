...
## 40. 감시·자동 매매 헬퍼 — 설계 (2026-09-26, 0·1단계 구현)
...
- `triggers` (0009, 동작이 알림·주문 둘이라 order_ 를 뺐다): 사용자, id, 이름, 대화 id, 소스(JSON), 동작(JSON), 한도, 만료, 상태, 사용 횟수, 마지막 신호·평가 시각
...
1. 자체 감시 + 알림 동작 (Binance 봉, 30일 미리보기, `watch_alert` 툴) — ✅ 구현 (2026-09-26). 아래 "1단계 구현"
...
- **조건 평가** (`triggers/condition.ts`, 순수 함수): 발동 = `streak(i) === confirmBars` (i 에서 끝나는 연속 충족 봉 수). 조건이 유지되면 streak 가
  confirmBars 를 넘어가 다시 울리지 않고(on_enter), 거짓 → 참이면 또 울린다. **상태를 저장하지 않고 봉 배열만으로 정해져서** 감시기는
  "마지막으로 평가한 봉" 만 기억하고, 재기동 뒤 놓친 봉도 같은 규칙으로 소급한다. 값 13종(`indicators.ts` 재사용 + 직전 20봉 대비 거래량 배수), 비교 6종, AND 최대 5개.
- **봉** (`bars.ts`): Binance klines, 진행 중인 마지막 봉은 뺀다. 실측 ETHUSDT 1시간봉 800개 요청 → 닫힌 봉 799개.
- **툴 `watch_alert`**: prepare(확인 카드 + 지난 30일 미리보기 — 예열 80봉은 세지 않는다, 지금 이미 참이면 경고, 채널이 없으면 경고) · list · pause.
  대화 id 는 pi 툴 컨텍스트(`ctx.sessionManager.getSessionId()`)에서. 켜기 토큰은 주문 토큰 구조(서명·1회용)에 **10분**, 서명 키 분리.
- **켜기·다시 켜기 때 기준 봉을 지금으로** — 미리보기에 나온 과거 발동이나 멈춰 있던 동안의 봉으로 켜자마자 울리지 않는다.
- **감시기** (`watcher.ts`): 10초마다 만료·비활성 계정 처리 → (종목, 간격) 으로 묶어 새로 닫힌 봉이 있는 묶음만 한 번 조회(봉 마감 + 3초) →
  트리거마다 마지막 평가 봉 뒤만 평가. 쿨다운·최대 발동(소진 시 done). **1봉 넘게 늦은 발동은 "늦은 알림" 하나로 묶는다.**
  조회 실패는 lastError 로 남기고 봉을 넘기지 않는다 (다음 점검에서 같은 봉을 다시).
- **조작은 `WatchOps` 한 곳** (앱·에이전트·텔레그램 공용, 권한도 여기서): 켜기·다시 켜기 = 앱만, 일시정지 = 셋 다, 삭제·비상 정지 = 앱·텔레그램.
  모든 변경은 `trigger_events` + 떠 있는 화면(`watch_event`). 상태 변경은 화면에만 (텔레그램으로 한 조작을 텔레그램에 다시 보내지 않는다).
- **텔레그램 명령** (`notify/telegram-bot.ts`): 사용자 봇마다 롱 폴링(25초). 저장된 개인 채팅에서 그 사람이 보낸 것만 처리, 2분 넘은 명령은 버린다(재기동 뒤 쌓인 것).
  /list(일시정지·삭제 버튼) · 삭제는 두 번(확인 버튼 10분) · /stop(비상 정지, 확인) · 그 외 문장은 "만들기는 앱에서". 409(다른 곳이 읽는 중)는 1분 뒤 재시도하고 설정 화면에 표시.
  연결 테스트 동안은 폴링을 멈춘다 (getUpdates 가 겹치면 409).
- 웹: 켜기 확인 카드(조건·마지막 마감·미리보기·받는 곳·횟수/만료), 설정 → 감시 탭(목록·일시정지·다시 켜기·삭제·비상 정지·최근 기록·만든 대화),
  발동 토스트(누르면 만든 대화).
- 테스트: 조건·봉·툴 15개, 저장·감시기·권한 14개, 텔레그램 7개. 뮤테이션 14/14. 부팅 스모크(D1 없이): 라우트·툴 등록(`watch_alert`).
- 아직: "만든 대화에 결과 기록" (지금은 토스트·텔레그램·기록 탭의 링크), 주식 봉(토스·KIS), 주문 동작(2단계).

### 주식·봉 간격 확장 설계 (2026-09-26)

요구: 국장·미장, 봉 5분 · 10분 · 15분 · 30분 · 1시간 · 2시간 · 4시간 · 1일 · 1주, "직전 봉 대비 거래량 %", 프리·애프터는 옵션 (급락 방어).

TradingView 확인 (공식 문서):
- 시간 간격에 일(D) 1~365 와 주(W) 1~52 가 따로 있고, 봉은 경계가 정해진 구간이다 — "최근 7일을 굴리는 창" 이라는 봉은 없다 → **주봉(1W)**.
- 세션은 차트마다 정규장 / 확장 시간 선택, 분·시간봉에만 적용. 미장 1시간봉은 정규장이면 09:30 기준, 확장이면 정각 기준.
  지표 알림은 만들 때의 설정을 그대로 쓴다.
- Relative Volume at Time: 기준 구간(Anchor) 시작부터 지금 봉까지의 경과 시간이 같은 지난 N개 구간의 봉 평균과 비교 (지금 ÷ 평균).
  누적(기본)·일반 모드. 같은 지점 봉이 없으면 그 직전 봉.

결정:
| 항목 | 결정 |
|---|---|
| 간격 | 5m · 10m · 15m · 30m · 1h · 2h · 4h · 1d · 1w (코인은 1m 도 — 1단계 호환). Binance 에 10분봉이 없어 5분봉 2개를 묶는다 |
| 봉 경계 | 코인 UTC 시계 (주봉 월요일 00:00 UTC — epoch 0 이 목요일이라 단순 나눗셈이면 어긋난다). 주식은 장 기준 — 국장 KRX 09:00–15:30, 미장 09:30–16:00 뉴욕(서머타임은 Intl) |
| 세션 | \`session: regular(기본) | extended\` — 주식 분·시간봉만. 일봉·주봉은 정규장 |
| 거래량 | \`vol_chg_pct\`(직전 봉 대비 %, 일봉·주봉에 알맞음) · \`rvol\`(TradingView Relative Volume at Time 정의, 기준 = 그날 장, 기본 N=10, 누적·일반) |
| 장중 봉 거래량 주의 | 장중 거래량은 U자형 — 1시간봉 직전 봉 대비는 마감 봉에서 매일 울리고 점심엔 안 울린다. 장 첫 봉은 전날 마지막 봉과 비교하지 않는다(null) |
| 출처 | 트리거 주인의 증권 키. KIS 우선(국장 KRX/NXT/통합 선택 가능, 미장 분갭 NMIN) → 토스(1분·일봉, 묶기) |

단계: ① 국장·미장 일봉·주봉 + vol_chg_pct ✅ ② 미장 분·시간봉 (KIS 분갭) ③ 국장 분·시간봉 (1분봉 버퍼·장 기준 묶기) ④ rvol ⑤ 확장 세션.

#### ① 구현 (2026-09-26)

- \`market-time.ts\` 시장 시계: 마지막으로 닫힌 봉·봉 마감·다음 마감·늦은 알림 기준(코인 1봉, 주식 일봉 12시간·주봉 2일)·평가 지연(코인 3초, 주식 10분 — 동시호가·집계 여유).
  주식은 **캘린더 없이** 평일 기준 "닫혔어야 할 봉" 을 계산한다 — 휴장일엔 그 봉이 안 올 뿐이라 판정이 틀리지 않는다. 감시기는 그런 묶음을 10분(코인 30초) 동안 다시 조회하지 않는다.
  알려진 한계: 국장 수능일(16:30 마감)·미장 조기 폐장(13:00)은 고정 시각으로 본다 (조기 폐장은 늦게 평가할 뿐, 수능일은 진행 중 봉일 수 있다) → 분봉 단계에서 장 캘린더 연동.
- \`stock-bars.ts\`: 일봉을 KIS(국내 날짜 구간·해외 기준일 이어 받기) → 토스(nextBefore) 순으로, 최대 6번. 주봉은 KIS 주봉 대신 **일봉을 월~금으로 묶는다**
  (봉 시각 = 그 주 월요일 장 시작 — 월요일 휴장이어도 같은 값). 진행 중인 오늘 일봉·이번 주 주봉은 뺀다.
- 감시기: 주식은 트리거 주인별로 묶어 각자의 키로 조회 (코인은 공개 시세라 모두 한 번). 예열 봉 수를 조건에 쓰인 값에 맞춘다(\`warmupFor\` — vol_chg_pct 2봉, RSI 45봉, ma60 60봉).
- 툴: \`market\`(비우면 심볼로 추정: 6자리 = 국장, USDT 등으로 끝나면 코인, 그 외 미장), \`session\`, 미리보기는 코인 분·시간봉 30일 · 일봉 약 6개월(주식 120봉) · 주봉 52봉.
- 테스트: 시장 시계(국장 마감 전후·주말·미장 서머타임 전후·전환일 새벽·주봉 경계), 일봉→주봉, 진행 중 봉 제외, 10분봉 묶기, vol_chg_pct, 예열,
  감시기(마감 10분 뒤·실제 삼성전자 09/18 +47.9%·휴장일 헛조회 방지·사용자별 조회·다음 날 아침 늦은 알림). 뮤테이션 11/11.
  실데이터: Binance ETHUSDT 10분·2시간·주봉 (간격 연속·주봉 월요일 시작·진행 중 봉 제외). **주식은 실제 키로 아직 안 돌렸다.**

### 조건 조합 · 프리셋 (2026-09-26)

전략을 코드로 하나씩 만들지 않고 **부품(값·비교·묶음)** 을 갖춘 뒤, 자주 쓰는 조합만 **프리셋(데이터)** 으로 둔다. 평가기는 하나, 프리셋은 켤 때 조건으로 펼쳐 저장한다.

- **값**: 이름(\`rsi14\` 등 1단계 호환 — D1 에 그 모양으로 저장돼 있다) 또는 매개변수 값
  \`sma·ema(period, of)\` · \`rsi(period)\` · \`highest·lowest(period, of, offset — 기본 1 = 지금 봉 제외. 포함하면 종가가 자기 고가를 못 넘어 돌파가 영원히 안 된다)\` ·
  \`change_pct(period)\` · \`vol_ratio(period)\` · \`rvol(length=10, mode=cumulative|regular)\` · \`value(of, offset)\`. 모든 값에 \`mul\`(배수).
- **rvol**: TradingView Relative Volume at Time 정의 그대로 — 기준 구간 = 그날(코인 UTC 00:00, 주식 장 시작), 같은 오프셋(없으면 그 직전 봉)의 지난 length 일 평균 대비.
  일봉 이상은 구간이 봉 하나라 직전 length 봉 평균 대비. 예열 = (length+1) × 하루 봉 수 (5분봉 10일 = 3,168봉).
- **묶음**: \`any\`(OR) · \`all\`(AND) · \`within: N\`(최근 N봉 안에 한 번이라도). 3값 논리 — AND 는 하나라도 거짓이면 거짓, OR 는 하나라도 참이면 참, 나머지 모름이면 모름.
  (1단계는 AND 에 모름이 섞이면 모름이었다 — 발동 결과는 같다, 둘 다 연속 봉을 끊는다.)
- **한도**: 절 10개, 깊이 3, 기간 500, within 50, rvol 30일. 예열 봉 수가 시장별 상한을 넘으면 거절 — 코인 5,000봉(Binance endTime 이어 받기, 1,000개씩),
  주식 일봉 550 · 주봉 110.
- **프리셋 10종** (\`presets.ts\`): 거래량 급증 돌파 · 직전 봉 대비 거래량 급증 · 같은 시각 대비 거래량 급증 · 신고가 돌파 · 과매도 반등 · 골든/데드크로스 ·
  볼린저 하단 이탈 · 급락 방어(마감 손절, 기준가 필수) · 급등·급락(방향 0 = OR). 매개변수마다 기본값·범위, 모르는 매개변수는 거절. 조건에 \`preset: { id, params }\` 를 표시용으로 남긴다.
- 알림 값 문구도 설명문과 같은 이름 (\`valuesText\` — \"종가 20 · 2봉 이평 × 1.5 22.5\").
- 테스트 +17 (매개변수 값·돌파 offset·rvol 누적/일반/직전 봉 대체/일봉·any/within 모름·검증 한도·설명문·프리셋 전부 펼쳐서 검증·툴·이어 받기 경계). 뮤테이션 14/14.
- 실데이터 (Binance ETHUSDT): rvol 5분봉 3,668봉 이어 받기 0.75초, 조합(RSI(7) 또는 볼린저) 1시간봉, 프리셋 10종 4시간봉.
  **\"직전 봉 대비 거래량 +50%\" 를 4시간봉에 걸면 500봉 중 107번** — 장중 봉에는 맞지 않는다는 걸 미리보기가 그대로 보여 준다 (프리셋 설명에 rvol 을 권한다고 적었다).

### 주식 시세 출처 고정 (2026-09-26, 실측)

실제 키로 \`spike/11-watch-stocks.ts\` (운영과 같은 D1 토큰 저장소 — 캐시가 살아 있어 KIS 토큰 재발급 없음):
- 이어 받기: 삼성전자 KIS 388일 · 토스 400일, AAPL KIS 300일 · 토스 400일. 감시 경로로 삼성전자 09/18 +47.9% 발동, 미장 일봉 22:30(뉴욕 09:30) 시작, 추석(9/24–25) 뒤 주봉 닫힘.
- 주봉 묶음 ↔ KIS 주봉: 거래량 4주 모두 일치. 09/21 주 종가만 다르다 (우리 286,500 = 9/23 종가, KIS 주봉 285,500 — 원인 미확인).
- **국장 거래량이 출처마다 다르다**: 토스가 KIS 의 1.4~1.7배. KIS 기간별시세에 \`FID_COND_MRKT_DIV_CODE=UN\` 을 주니 토스와 종가·거래량이 정확히 같다
  → 토스 = KRX+NXT 통합, KIS 기본(J) = KRX 만. **통합 봉은 가격도 다르다** (09/22 종가 276,500 → 277,500, 저가 274,500 → 271,500 — NXT 프리·애프터 체결 포함,
  종가 = 20시 체결가). 미장은 두 출처 거래량이 같다.
- 결정: 트리거마다 **출처를 켤 때 고정** (\`market.feed = { provider, basis }\`). 감시기는 그 출처로만 조회하고, 키가 없거나 실패하면 다른 출처로 넘어가지 않고 오류.
  국장 기준: \`krx\`(KRX 정규장, KIS J, 15:30 마감 — 기본, KIS 키 필요) · \`integrated\`(KRX+NXT 통합, KIS UN 우선 · 토스, **20:00 마감**). 토스만 있으면 통합만.
  미장은 출처만 고정 (KIS 우선). 예전 트리거(feed 없음)는 KIS → 토스 그대로. 카드에 \"시세 기준\", 조건 설명에 \"(KRX+NXT 통합)\".
- 테스트 +5 (기본값·통합 20:00 마감·고정 출처 엄격·KIS UN 코드·툴 카드), 뮤테이션 5/5. 실측: KIS 통합 = 토스 통합 (종가·거래량), KRX 기준과 다름.
