...
- [ ] 툴 수 관리 — 현재 커스텀 29개 (mcp_call 포함, 2026-09-24 실측 — 이전 "23개" 는 오래된 값). 더 늘리기 전에 통합 검토
- [ ] `market_timing` 손익비 기준(현재 1.0) — 실사용 후 조정 여부 판단
- [ ] KIS 주문 — hashkey, tr_id 분기 (국내 TTTC0011U/0012U, 해외 TTTT1002U/1006U + 거래소별), 모의계좌로 먼저
- [ ] 토스 정정 주문, OCO 조건주문
- [x] **사용자별 MCP 서버 설정 + MCP OAuth 클라이언트** — 자체 브릿지 `packages/mcp`, `mcp_call` 게이트웨이 1개 (PLAN §38)
  - SDK 대신 직접 구현 (서버·stdio 의존 17개 회피, SSRF 방어를 연결 단계에). 원격 https 만, DNS 해석 주소 검사
