# TODO

기능 개발은 `stock_research`(dda3029)에서 일단 멈춤. 설계·배경은 PLAN.md 해당 절 참고.

## 계정 (PLAN §23)

- [x] 가계부 분리 · 멤버 · 초대 (마이그레이션 0006)
- [x] 회원가입 — 1회용 초대 코드, env 계정 = 슈퍼관리자, 관리자 화면 (PLAN §25)
- [x] 토큰 무효화(token_version), 비밀번호 변경, 모든 기기 로그아웃, 임시 비밀번호
- [ ] 가입·관리자 화면 실제 조작 확인 (웹·iOS)
- [ ] 계정 삭제 (지금은 비활성화만 — 가계부 소유권 처리를 정해야 한다)
- [ ] 2FA(TOTP) — 완전 선택, 복구 코드
- [ ] 받은 초대 배지를 사이드바·탭에 — 지금은 가계부 화면 맨 위 카드뿐
- [ ] 가계부 화면 실제 조작 확인 (웹·iOS) — 타입·빌드만 확인, 화면은 사람 손
- [ ] `transactions.dedupe_key` 가 전역 UNIQUE — 가계부 간 충돌 가능. 임포트를 만들 때 `(ledger_id, dedupe_key)` 로
- [ ] 구 데이터(`budgets_legacy` 1행, spike 잔여) 삭제

## 대화 (PLAN §24)

- [x] 서버: 대화별 세션, 백그라운드 응답, /api/sessions
- [x] 웹: `/c/<id>` 주소, 사이드바 대화 목록 (응답 중·새 답 표시)
- [ ] 툴 진행 문구(flavor line) 불일치 — ledger_add 에 "종목 목록을 훑어봤어요" 가 붙음
- [ ] 대화 이름 바꾸기·삭제, 서버 재시작 시 진행 중이던 응답은 사라짐 (재개 불가)
- [ ] 백그라운드 완료 푸시 알림 (iOS 푸시 작업 때)

## 이미지 첨부

- [x] 서버: 검증(최대 4장·4MB·매직 바이트), 모델 직접 전달, 못 읽는 모델이면 미리 거절, 이미지 속 지시 무시 규칙
- [x] 웹: 첨부 버튼·붙여넣기·끌어놓기, 1600px JPEG 축소, 말풍선에 첨부 표시
- [ ] iOS 실기기에서 카메라·사진 선택 확인 (권한 문구는 Info.plist 에 추가됨)
- [ ] 매 message_end 가 대화 전체(이미지 base64 포함)를 다시 보낸다 — 이미지가 많아지면 참조 URL 방식으로
- [x] 스모크 간헐 실패 — 64px 단색 이미지 검사가 원인, 영수증 픽스처로 교체 (PLAN §26)

## 기능 (보류)

- [ ] **`monthly_report(month)`** — 스킬 툴화의 마지막 (PLAN §21)
  - 가계부: 월 합계, 전월 대비, 카테고리 증감, 예산 대비 — 지금 바로 가능
  - 투자 손익: `SnapshotStore.onOrBefore` 로 월초 기준값 조회. 스냅샷은 2026-09-23 16시부터 쌓이므로
    월초 기준값은 10월 이후 생긴다 → 없으면 "기준 데이터 없음"을 명시하는 구조로
- [ ] 툴 수 관리 — 현재 커스텀 30개 (watch_alert 추가) (mcp_call 포함, 2026-09-24 실측 — 이전 "23개" 는 오래된 값). 더 늘리기 전에 통합 검토
- [ ] `market_timing` 손익비 기준(현재 1.0) — 실사용 후 조정 여부 판단
- [ ] KIS 주문 — hashkey, tr_id 분기 (국내 TTTC0011U/0012U, 해외 TTTT1002U/1006U + 거래소별), 모의계좌로 먼저
- [ ] 토스 정정 주문, OCO 조건주문
- [x] **사용자별 MCP 서버 설정 + MCP OAuth 클라이언트** — 자체 브릿지 `packages/mcp`, `mcp_call` 게이트웨이 1개 (PLAN §38)
  - SDK 대신 직접 구현 (서버·stdio 의존 17개 회피, SSRF 방어를 연결 단계에). 원격 https 만, DNS 해석 주소 검사
  - TradingView 읽기 허용목록 25개 (쓰기 10개 차단), 그 외 서버는 이름·힌트 판정. 역할 분담은 persona 에
  - 디스커버리·DCR(전역 1회 캐시)·PKCE·resource, 콜백 `/api/mcp/oauth/callback`, 단일비행 갱신, 해제 시 폐기
  - bearer/인증 없음 서버도 같은 경로. 설정 → 연결 → MCP 서버 (웹·앱)
- [x] 운영에 `AF_PUBLIC_URL` 설정 → TradingView 연결 확인 (읽기 25 · 쓰기 10)
- [x] MCP 쓰기(CUD)는 확인 카드로 — 준비만 하고 사람이 [확인] (PLAN §39)
- [ ] TradingView 알림 만들기 → 확인 → list-alerts 로 확인 → 삭제 실측 (사람 손)
- [ ] iOS 앱에서 시스템 브라우저 OAuth 흐름 확인 (`window.open` → Safari → 완료 페이지 → 앱 복귀 시 재조회)
- [ ] **감시·자동 매매 헬퍼** (PLAN §40) — 0단계 알림·텔레그램 ✅ / 1단계 자체 감시 + 알림 + 텔레그램 명령 ✅ / 2 주문(체결기) / 3 웹훅 / 4 토스·KIS 봉·APNs
- [ ] 감시 실사용 확인 — 챗에서 준비 → 켜기 → 발동 알림(텔레그램) → /list·일시정지·삭제 (사람 손)
- [ ] 감시 결과를 만든 대화에 기록 (지금은 토스트·텔레그램 링크만)
- [ ] 감시 주식 확장 (PLAN §40) — ① 국장·미장 일봉·주봉 + vol_chg_pct ✅ / ② 미장 분·시간봉 / ③ 국장 분·시간봉 / ④ rvol ✅(평가기, 주식 장중은 ②③ 뒤) / ⑤ 프리·애프터 세션
- [x] 감시 조건 조합 — 매개변수 값·배수·N봉 최고/최저·변동률·rvol, any·within, 프리셋 10종
- [ ] 사용자 프리셋 저장 ("내 프리셋"), 앱·텔레그램에서 프리셋 골라 만들기
- [x] 주식 감시 실데이터 확인 (spike/11) — 국장 거래량: 토스 = KRX+NXT 통합, KIS J = KRX 만 → 출처 고정 (PLAN §40)
- [ ] KIS 주봉 종가가 마지막 거래일 종가와 다른 이유 (09/21 주 285,500 vs 286,500)
- [ ] 텔레그램 실제 연결 확인 (사람 손 — 봇 만들기)
- [ ] MCP 결과 카드 렌더러 — 경제·실적 캘린더, 스크리너 (`details.kind`)

## iOS 앱 (Phase 5, PLAN §22)

- [ ] 시뮬레이터에서 실제 로그인 → 채팅·카드·주문 확인 카드 흐름 점검 (사람 손 — 입력이 필요)
- [ ] 실제 앱에서 로그인 후 재실행 시 Keychain 토큰 유지 확인, 로그아웃 시 삭제 확인
- [ ] safe area·키보드 — 다른 세션의 `viewport.ts` 작업과 합친 뒤 실기기 확인
- [ ] 앱 아이콘·스플래시 (지금은 Capacitor 기본 이미지)
- [ ] 번들 ID 확정 (현재 `com.alphafolio.app` — 첫 TestFlight 전에만 바꿀 수 있다고 보면 된다)
- [ ] 앱 번들에서 PWA 서비스워커 빼기 — WKWebView 에서 등록이 실패할 뿐 무해하지만 불필요 (`vite.config.ts`)
- [ ] 실기기 — 서명(팀 ID 는 커밋하지 않는다), HTTPS 서버 주소로 빌드 (`task mobile:build API_BASE=https://...`)
- [ ] Face ID 잠금 → 카메라(영수증) → 푸시 순

## 배포 (Phase 4)

- [x] Docker 이미지 실제 빌드 확인 — 로컬 arm64 빌드·기동 OK (툴 22개, 확장 로드, 코딩툴 미노출, uid 1001, healthy).
      `packageManager` 가 없어 corepack 이 pnpm 12 를 받던 것을 11.0.9 로 고정
- [x] GitHub Actions — `ci.yml` (검사 → arm64 이미지 → GHCR) + `bump.yml` (Bump & release, semver)
- [x] GHCR 패키지 공개 — 공개 저장소에 연결돼 public 으로 생성됨 (익명 pull 확인)
- [ ] iOS 앱 버전(MARKETING_VERSION)을 package.json 버전과 맞출지 — TestFlight 때 결정
- [ ] 이미지 1.16GB — `chown -R /app` 레이어가 node_modules 를 통째로 복제한다 (`COPY --chown` 으로 줄일 수 있음)
- [ ] D1 미설정 서버에서 스냅샷 스케줄러가 기동 때마다 "실패" 로그를 찍는다 — 브로커 미설정처럼 조용히 건너뛰기
- [x] 운영 배포 — 컨테이너 1개(이미지 버전 고정), `infra/compose.example.yaml` 한 파일 템플릿 → 서버 compose.yaml(600)에 값 채움, 포트는 127.0.0.1 에만
- [x] 프록시 뒤 클라이언트 IP — CF-Connecting-IP 우선, XFF 는 맨 끝 (맨 앞은 위조 가능)
- [ ] 위 IP 수정·thinking·관리자 변수가 들어간 버전으로 운영 이미지 올리기 (현재 운영 0.0.2)
  - ⚠️ 이 버전부터 운영 compose 의 `AF_USERS` 줄을 `AF_ADMIN_USER` / `AF_ADMIN_PASSWORD` 로 바꿔야 기동한다
- [ ] 기존 사용자 이전 — 가입 코드 발급 → 가입 → 각자 설정에서 증권 키 다시 입력
- [ ] 스모크의 `web_search` 를 실제 검색으로

## 공개 전

- [x] LICENSE (MIT)
- [ ] README·PLAN 공개 관점 재검토 (운영 정보 노출 여부)
- [x] 푸시 직전 시크릿 스캔 재실행 + 실계좌 보유·손익 수치를 히스토리에서 제거 — 양성 대조군을 넣어 스캔이 실제로 도는지 확인 (macOS xargs 에 -a 가 없어 첫 스캔이 빈손이었음)
- [x] spike 가 남긴 예산 테스트 행 — 0006 에서 `budgets_legacy` 로 격리됨 (spike 02 는 이제 전용 가계부를 만들고 지운다)

## 사람 손 (사용자)

- [ ] 토스 실주문 검증 — 장외 지정가 주문 → 취소, 이후 1주 체결 (PLAN §18)
- [ ] 네이버 뉴스 키 입력 → `market_news` / `stock_research` 뉴스 섹션 실측
- [ ] 단기 타점 순위 근거 — 돌파 진입은 손익비가 1.33 으로 같아 순위가 안 갈린다 (거래대금·상대강도·ATR% 등, PLAN §29)
- [ ] 타점 백테스트에 하락장 구간 추가 — 지금 2년치는 전부 상승장이라 추세 추종에 유리했다 (range 5y 로, PLAN §33)
- [ ] 단기(1주) 규칙은 기준선과 같다 — 신호 우위 없음. 상대강도·거래대금 급증 등으로 개선할지 백테스트로 (PLAN §33)
- [ ] 토스 조건주문으로 "돌파 시 매수" 준비 — 지금은 돌파 확인 후 사람이 다시 요청해야 한다
- [ ] KIS 외화 예수금 — 해외 체결기준현재잔고(CTRP6504R) 등으로 조회 (지금은 토스만, PLAN §30)
- [ ] 환율 출처 — KIS 환율(1,360)이 토스 실시간(1,368)보다 먼저 쓰인다. 해외 종목 원화 환산이 ~0.6% 낮게 나옴. 실시간 우선으로
- [x] 모든 API 노출 2차 — 토스 조회 29개 `toss_query` (PLAN §32)
- [ ] `order_prepare` 에서 매수 유의사항(getStockWarnings) 확인 → 투자경고·VI 종목이면 확인 카드에 경고
- [ ] 3차 — 쓰기: KIS 주문(국내·해외·예약·정정취소), 토스 정정·조건주문(OCO/OTO) — 모두 확인 카드 경유
- [ ] 4차 — finnhub · Twelve Data · CoinGecko · Binance (설정 키 칸 + 같은 구조, Binance 주문도 확인 카드)
- [ ] 5차 — KIS 실시간 웹소켓 61개 · 선물옵션 그릭스
- [ ] KIS 카탈로그의 단위 없는 필드(시가총액 등) — 실측으로 단위를 확인해 보강 표
